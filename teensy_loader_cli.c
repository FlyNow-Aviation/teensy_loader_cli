...
void usage(const char *err) {
  ...
  fprintf(stderr,
          "Usage: teensy_loader_cli --mcu=<MCU> [--vid=<VID>] [--pid=<PID>] "
          "[--serial=<SERIAL>] [-w] [-h] [-n] [-b] [-v] <file.hex>\n"
...
int main(int argc, char **argv) {
  ...
  if (!boot_only) {
    ...
    printf_verbose("Read \"%s\": %d bytes, %.1f%% usage\n", filename, num,
                   (double)num / (double)code_size * 100.0);
  }
    ...
    for (dev = bus->devices; dev; dev = dev->next) {
      ...
      if (serial) {
        if (dev->descriptor.iSerialNumber) {
          int ret = usb_get_string_simple(h, dev->descriptor.iSerialNumber,
...
int soft_reboot(void) {
  ...
  int response = usb_control_msg(serial_handle, 0x21, 0x20, 0, 0,
                                 reboot_command, sizeof reboot_command, 10000);
...
HANDLE open_usb_device(int vid, int pid) {
  ...
  for (index = 0; 1; index++) {
    ...
    SetupDiGetInterfaceDeviceDetail(info, &iface, NULL, 0, &required_size,
                                    NULL);
    details = (SP_DEVICE_INTERFACE_DETAIL_DATA *)malloc(required_size);
    if (details == NULL) continue;
    memset(details, 0, required_size);
    details->cbSize = sizeof(SP_DEVICE_INTERFACE_DETAIL_DATA);
    ret = SetupDiGetDeviceInterfaceDetail(info, &iface, details, required_size,
                                          NULL, NULL);
    if (!ret) {
      free(details);
      continue;
...
void init_hid_manager(void) {
  ...
  if (hid_manager == NULL ||
    ...
    printf_verbose(
        "no HID Manager - maybe this is a pre-Leopard (10.5) system?\n");
    return;
  }
...
int teensy_write(void *buf, int len, double timeout) {
  ...
  while (CFAbsoluteTimeGetCurrent() - timeout < start) {
    ret = IOHIDDeviceSetReport(iokit_teensy_reference, kIOHIDReportTypeOutput,
                               0, buf, len);
...
int hard_reboot(void) {
  ...
  rebootor = open_usb_device(hard_vid, hard_pid);
  if (!rebootor) return 0;
  ret = IOHIDDeviceSetReport(rebootor, kIOHIDReportTypeOutput, 0,
                             (uint8_t *)("reboot"), 6);
  ...
  if (code != 0) {
    ...
    if (code == 4 && len == 2) {
      ...
      if (code_size > 1048576 && block_size >= 1024 &&
          ...
          extended_addr < 0x60000000 + code_size) {
    ...
    if (strncmp(arg, "-mmcu=", 6) == 0) {
      ...
      if (arg[1] == '-') {
        ...
        if (strcasecmp(name, "help") == 0)
          ...
          list_mcus();
        else if (strcasecmp(name, "vid") == 0)
          hard_vid = soft_vid = strtoul(val, NULL, 16);
        else if (strcasecmp(name, "pid") == 0)
          hard_pid = soft_pid = strtoul(val, NULL, 16);
        else if (strcasecmp(name, "serial") == 0)
          opt_serial = val;
        else {
